...
\begin{equation}\label{eq:Nequal1gauge}
  ...
  \end{aligned}
\end{equation}
We note that the action~\eqref{eq:startingaction} defines a variational problem adapted to Dirichlet boundary conditions for $B$ while the dual action~\eqref{eq:liftnonperturbative} defines a variational problem adapted to Neumann boundary conditions. In the specific case for which the action is not lifted by non perturbative effects, this describes a massless pseudo-scalar field for which $\Delta_+=d\,,\Delta_- = 0$. In Neumann quantization scheme, the conformal dimension of the dual operator is $\Delta_-=0$ so this describes a relevant deformation from the dual CFT perspective. In such a case, the charge $Q\sim a^d \partial_\tau \chi$ is a constant of motion and is related to the source parameter in Neumann quantization scheme (there is no VEV in this case). Once included non-perturbative effects, the charge is no longer a constant of motion and the variational problem does not correspond to fixing the charge at the boundary.
...
\paragraph{Frieman Vaga variables} \paragraph{Frieman Vaga variables} For the conventions, we refer to https://arxiv.org/pdf/astro-ph/0004138. We assume our potential+cosmological constant is given by $V(\chi) = V_0 + V_1 \cos\left(\frac{\chi} {f_\chi}\right)$. There are five possibilities:
\begin{itemize}
...
\paragraph{Holographic properties of the solutions.}
...
\end{equation}
For sufficiently small $\alpha^2(1-v_0)$, the BF bound is violated. Consider the case where the BF bound is satisfied.  Let us show that the charge of the axion blows up at the AdS boundary. To see why, first observe that the scale factor behaves (in units of AdS length) as $a(\tau)\sim e^{\tau}$. We then expand the axion field as
\begin{equation}
  \chi(\tau) = \chi_{(-)} e^{-\Delta_- \tau} + \chi_{(1)}e^{-\left(\Delta_- + 1\right)\tau} + \cdots + \chi_{(+)} e^{-\Delta_+ \tau} + \cdots\,,
\end{equation}
where in Neumann quantization shceme, $\chi_-$ corresponds to the VEV of the dual operator and $\chi_{(+)}$ corresponds to the source. We first remark that the finiteness of the charge $Q(\tau)\sim a^d \partial_\tau \chi$ of the boundary imposes the vev to vanish. Then, since the deformation is relevant $\Delta_+ \leq d$ where the equality turns off the potential and we are eventually back to the non-running solutions. We conclude that the charge of the axion blows up at the AdS boundary for running solutions. The solutions describe infinite flux at the AdS boundary.
...
\begin{equation}
  \mathcal{S}_{\rm ct} = \frac{1}{\kappa}
\end{equation}
