...
\end{equation}
the orbits will circle around the fixed point before reaching it. Otherwise, the orbits will directly flow to the fixed point. We can check that numerically by displaying the two type of solutions. The next step is to draw the phase portrait. Note that the $\chi$ particle describes a motion similar to a disspative pendulum whose friction/anti-friction coefficient is time-dependent. This is natural to project the phase portrait into the $(I,\mathcal{J}) \sim (\chi, \Pi_{\chi})$.
\end{document}
