...
In parallel, we can numerically integrate the first two equations of~\eqref{eq:Nequal1gauge} which we recall here
\begin{equation}
  \begin{aligned}
    \partial_\tau\left(a^d\partial_\tau \chi\right)-\lambda f_{\alpha} a^{d}\sin(\chi f_{\alpha})&=0\,.\\
    \frac{d(d-1)}{a^2}\left(a'^2-k\right)+2\kappa\left(\frac{(\partial_\tau \chi)^2}{2}+\lambda \cos(\chi f_{\alpha})+\frac{\Lambda}{\kappa}\right)&=0\,.
  \end{aligned}
\end{equation}

We want to solve for $(\chi,a)$. Note that these equations are unchanged under a time reversal transformation. It should be sufficient to give as initial conditions the value of $\chi$ which we set at a local maximum of the potential at $\chi=0$ WLOG. We also give an initial value for the scale factor $a(\tau=0) = \bf{a}>0$ as well as a velocity of the $\chi'(\tau=0) = \chi_0'$ particle.
\PG{You cannot consistently impose $\chi'(0)=0=a'(0)$ to obtain non trivial $\mathbb{Z}_2$ even solutions. Indeed, the first equation imposes $\sin(\chi f_{\alpha})=0$ at the initial time so $\chi$ lies at an extremum. Then, the second equation gives the value of the scale factor wrt $\Lambda_{\rm eff}$.}
\end{document}
