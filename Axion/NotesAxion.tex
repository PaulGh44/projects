...
\paragraph{Holographic properties of the solutions} In this paragraph, we describe the holographic properties of the domain-wall solutions which asymptotes to the EAdS fixed point. First note that near the AdS fixed point
\begin{equation}
  V(\chi)\to V_0 -V_1 <0\,.
\end{equation}
The spacetime asymptotes to an AdS geometry with length $L_{\rm AdS}$ defined by 
\begin{equation}
  L_{\rm AdS} = \frac{1}{M|\mathcal{H}_\star|} = \sqrt{\frac{1}{\kappa V_1}\frac{d(d-1)}{2(1-v_0)}}\,.
\end{equation}
Expanding at second order, one obtains
\begin{equation}
  V(\chi) = V_0-V_1 - \frac{V_1}{2}\left(\frac{\delta\chi}{f_\chi}\right)^2\cos(\mathcal{I}_\star)= V_0-V_1 + \frac{V_1}{2 f_\chi^2} (\delta\chi)^2 + \mathcal{O}((\delta\chi)^4)\,.
\end{equation}
This describes a massive deformation of mass
\begin{equation}
  m_{\rm holo}^2 =-m^2_{\rm axion} = - \frac{V_1}{f_\chi^2} = -\frac{M^2}{\alpha^2} \Rightarrow m_{\rm holo}^2L_{AdS}^2 = -\frac{d(d-1)}{2\alpha^2}\frac{1}{1-v_0}<0\,.
\end{equation}
From the point of view of the dual theory, choosing the standard quantization picture, it describes an relevant deformation as $\Delta_+ <d$. This potentiall breaks the BF-bound. If it satisfies the BF-bound, both modes are normalizable.
\end{document}
