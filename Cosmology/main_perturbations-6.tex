...
Also~\cite{Gazeau_2000,tolley2002quantizationmasslessminimallycoupled,Kirsten_1993,Folacci_1992} related to the de Sitter invariant vacuum problem for massless excitations.
...
\emph{Left:} Representative contours in the complex $t$ plane for the two orientations of the Wick rotation that lead to the same expanding Lorentzian geometry. The solid and dashed Euclidean segments correspond to $(\sigma,\eta)=(+,+)$ and $(-,+)$, respectively, and run from $t=\pm i\pi/(2H)$ to the turning surface at $t=0$. Both contours then continue along the positive or negative real axis depending on the orientation $\sigma$.
...
\begin{equation}
    \epsilon_V = \frac{1}{2 \kappa} \left(\frac{V_{\phi}}{V}\right)^2\ll1\,,\qquad \eta_V = \frac{1}{ \kappa} \frac{V_{\phi \phi}}{V}\,, \qquad \epsilon_V,\,\eta_V \ll 1\,.
\label{eq:slow_roll}
\end{equation}
...
\begin{equation}
    \Psi_{HH} \propto \exp\left(\frac{4\pi^2}{\kappa H^2}\right)\cos\left[ \frac{4\pi^2}{\kappa H^2}\left((bH)^2-1\right)^{3/2}-\frac{\pi}{4}\right]\,.
\end{equation}
...
\end{equation}
Thus, although the Hartle--Hawking and tunnelling wave functions are constructed from the same set of complex no-boundary saddles, they differ in the prescription used to select and combine them.


Alternatively, the boundary conditions may be discussed in the Lorentzian path-integral formulation. Schematically, in the minisuperspace truncation, the metric is written as
\begin{equation*}
    \rm{d}s^2 = - \frac{N^2}{q^2(t_q)} \rm{d}t_q^2 + q^2(t_q) \rm{d}\Omega_3^2\,,
\end{equation*}
and the wavefunction is given by a path-integral over the lapse function $N(t_q)$, the variable $q(t_q)$ and the scalar field $\phi(t_q)$
\begin{equation*}
    \Psi
    \propto
    \int_{\mathcal{C}_N}{\rm d}N
    \int_{B_i}^{B_f}
    \mathcal{D}q\,\mathcal{D}\phi\,
    e^{i\mathcal{S}_L[q,\phi,N]}\,.
\end{equation*}
In the de Sitter limit, where the scalar is frozen at a constant value for which the potential is positive, the path-integral over $q$ is Gaussian and can be performed exactly. The remaining integral over the lapse function $N$ is then evaluated in the saddle-point approximation. The saddle points are precisely the complex no-boundary instantons discussed above. From this perspective, which complex saddles actually contribute is determined by the choice of lapse contour $\mathcal{C}_N$ and by the boundary conditions. Different contour prescriptions can therefore select different combinations of the available saddle points. For further discussions, see~\cite{PhysRevD.39.2206,Lehners:2023yrj,Vilenkin_2018,Feldbrugge:2017kzv,
...
\section{Minisuperspace Wheeler-DeWitt wavefunction in Einstein-scalar theory}\label{app:WDW}
...
\end{equation}
We may further assume that the potential satisfies is slowly varying $\tilde{V}(\tilde{\phi}) \approx H^2$. We thus neglect derivatives with respect to $\tilde{\phi}$ at firstobtaining a one-dimensional minisuperspace Wheeler-DeWitt equation
\begin{equation}
...
\begin{equation}
     \Psi_{HH}(a)\propto\cos\left[\frac{4\pi^2}{\kappa H^2}\left((aH)^2-1\right)^{3/2}-\frac{\pi}{4}\right]\,.
\end{equation}
The Tunneling proposal consists in selecting only outgoing modes in the classically allowed region
\begin{equation}
    \Psi_{T}(a)  \propto \exp\left[-\frac{4\pi^2}{\kappa H^2}i\left((aH)^2-1\right)^{3/2}\right]\,.
\end{equation}
...
\end{equation}
Below the Planck scale, the second term is negligible and the Tunneling wavefunction is dominated by $\Psi_+^{(2)}(a)$ in the level of approximation we are working with.
...
\end{equation}
Hence, the Hartle-Hawking WKB function in the classically allowed region reads
\begin{equation}
    \Psi_{\rm HH}(a,\tilde{\phi}) \propto \exp\left[\frac{4\pi^2}{\kappa \tilde{V}}\right]\cos\left[\frac{4\pi^2}{\kappa \tilde{V}}((a \tilde{V})^2-1)^{3/2}-\frac{\pi}{4}\right]\,.
\end{equation}
Similarly, for the Tunneling wavefunction, applying the same procedure, one easily finds
\begin{equation}
    \Psi_{T}(a,\tilde{\phi})  \propto \exp\left[-\frac{4\pi^2}{\kappa \tilde{V}}\left(1+i\left((a\tilde{V})^2-1\right)^{3/2}\right)\right]\,.
\end{equation}
