...
Consider the action~\eqref{eq:simpleentanglemententropy} and a cut in $\phi$ space. We further consider a discretize version of the model in $\phi$-space and view $X\to iX$ as the time direction.
\begin{equation}
  H = \frac{1}{2a}\sum_{i=1}^{N} \pi_i^2 + \left(\phi_i -\phi_{i+1}\right)^2 + 4 \pi \tilde{\mu} e^{b \phi_i} \phi_i^2\,.
\end{equation} 
