...
\subsection{Wheeler De Witt constraint and physical states}
...
\end{equation}
where both $\ket{\Psi_L},\,\ket{\Psi_X}$ are primary states. Upon acting with Virasoro generators, such states span the hilbert space of the gravitational theory. Nevertheless, the Hilbert space is the truncation of this bigger space by the Wheeler-De Witt constraint
\begin{equation}
  \left(L_0^{(\phi +X)}+\bar{L}_0^{(\phi+X)}-2\right)\ket{\Psi}=0\,,
\end{equation}
where $L_0^{(\phi +X)}$ is the Virasoro generator on the plane. This condition applied on the tensor product of primary states~\eqref{eq:primarystates} imposes the marginality condition $\Delta_L +\Delta_X =1$. To inspect the form of the Wheeler-De-Witt equation, we first note that there is no interaction between the matter theory and the Liouville by assumption so that the total Hamiltonian is a tensor product. We focus on the Liouville side which is the non-trivial side (\PG{The other side tells you that $\ket{\Psi_X}$ is represented by a vertex operator in $X$-space, right ?}). On genus $0$, we can inspect the Wheeler De Witt equation by truncating to minisuperspace that is truncating to zero mode on the cylinder $\phi(\tau,\sigma) = \phi_0(\tau),\, \sigma \in [0,2\pi)$. The lagrangian density becomes a lagrangian
\begin{equation}
...
\paragraph{Third quantized description in genus 0 minisuperspace}
We focus in the case $c_L \geq 25$. The closed string Liouville states that we discussed earlier should be viewed as wavefunction of the universe: precisely, they describe the amplitude of a universe to be nucleated from a vertex insertion of momentum $p$. We consider a second quantized description of the minisuperspace theory which was already a second quantized description therefore defining a so-called third quantized description. Remembering that the Hamiltonian acting on $X$ is simply $\mathcal{H}_X = -\frac{\partial^2}{\partial X^2}$, the third quantized description should be
\begin{equation}
...
\end{equation}
We shall view this as a massive field theory whose mass is expressed in terms of the string coupling $m^2 \sim g_s(\phi)$ which depends on the position in the $\phi$ space. In particular, in the weak coupling region, it describes a massless scalar field theory in 1+1d. We therefore immediatly obtain that the entanglement entropy of a cut $A=[\phi_1,\phi_2]$ in the weak coupling region is given by Cardy-Calabrese universal law
\begin{equation}
...
From the KPZ relation~\eqref{eq:KPZ}, we expect that the final expansion should be expressed in terms of the cosmological constant of the continuum Liouville theory $\mu$ after taking the continuum limit. We thus expect the following relation 
\begin{equation}
  \mu =  \left(\kappa_c^2-\kappa^2\right)\,,
\end{equation}
and the continuum cosmological constant is related to the lattice cosmological constant by
\begin{equation}
  \lambda_{\rm bare} \sim \frac{\mu}{a^2}\,.
\end{equation}
...
\end{equation}
Solving the eigenvalue problem yields an orthonormal basis with respect to the non-flat measure. Setting $\Psi = \psi \Delta(\lambda)$ produces a fermionic wavefunction. We can put in one to one correspondence to a theory of $N$ free fermions. The lowest energy configurations do not interact as the interaction is manifestly positive. The eigenvalue problem for the fermions reads
\begin{equation}
...
\end{equation}
The ground state is obtained by filling the N first lowest energy levels
\begin{equation}
...
\end{equation}
$\lambda_\pm$ are the turning points of the classical orbits. Clearly, if one tunes $\mu_F$ close to the quadratic local maximum of the potential, $\kappa(\mu_F)$ develops a non analytic behavior.
