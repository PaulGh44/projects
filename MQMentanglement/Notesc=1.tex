...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
In particular case of interest $z_{\shortrightarrow 1}=z,\,z_{\shortrightarrow 2}=z-1,\,z_{\shortrightarrow 3} = z-z_{\infty}$ with $z_{\infty}\to \infty$, this greatly simplifies leaving
...
\end{aligned}
\end{equation}
In the second line, we deform the contour sitting at $z$ ($L_{-2}^{(z)}=\frac{1}{2\pi i}\int_{w \sim z}\frac{dw T(w)}{w-z}$) to pick up the poles at the other positions. In the third line, we used $\partial_{z_i}=-\partial_{z_{\shortrightarrow i}}$. Sending the insertion points to $0,1,\infty$ and defining the rescaled correlation function, we obtain from~\eqref{eq:veryuseful}
\begin{equation}
  0 =\frac{1}{b^2}\partial_z^2 \mathcal{V}_{4} +\left[\frac{\Delta_1}{z^2}+\frac{\Delta_2}{(z-1)^2}-\left(\frac{1}{z}+\frac{1}{z-1}\right)\partial_z +\frac{2\Delta_{\infty} -\Delta_{\rm TOT}}{z(z-1)}\right]\mathcal{V}_{4}.\\
\end{equation}
After massaging the last equation, this yields
\begin{equation}
  0 =\frac{z(1-z)}{b^2}\partial_z^2 \mathcal{V}_{4} +(2z-1) \partial_z \mathcal{V}_{4} + \left[\Delta_{2,1}-\Delta_{\infty}+\frac{\Delta_1}{z}+\frac{\Delta_2}{1-z}\right]\mathcal{V}_{4}.
\end{equation}
