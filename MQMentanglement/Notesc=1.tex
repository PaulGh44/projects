...
\end{equation}
and then the integral over the matter field $X$ in an sum over all the embedding on the real line at each vertex. Finally, denoting $a$ the size of the sides of the triangles, we may assume the following dictionnary
\begin{equation}
\begin{aligned}
    \int_{\mathcal{M}_h} d^2 x \sqrt{g}g^{\mu \nu}\partial_\mu X \partial_\nu X &\rightarrow \sum_{\braket{ij}} \left(\frac{X_i-X_j}{a}\right)^2\,.\\
...
\end{equation}
with $\tilde{\kappa}=\sqrt{\frac{N}{\alpha}}$. Sending $\beta \to \infty$, the partition function can be formally written as an infinite sum over all Feynman diagrams (connected and disconnected)
\begin{equation}
  \lim_{\beta\to\infty}\log(Z) =\sum_{h=0}^{\infty}N^{2-2h}\sum_{\Lambda}\tilde{\kappa}^V \prod_{i=1}^V \int_{-\infty}^{+\infty} d\tau_i \prod_{\braket{ij}} e^{-|\tau_i-\tau_j|}\,.
\end{equation}
We highlight that to recover the partition function of the gravitational model we shall consider the generating functional of the connected diagrams i.e $\log(Z)$. The first complain is that the link factor is different from the matrix model compared to the discretized gravitational model. This is part of the universality statement that in the continuum limit, multiple link factors can describe the same continumm theory. We may associate the following dictionnary between the discretized gravitational model and the 
\paragraph{Solving the matrix model}
Also, thes partition function can be interpreted as a thermal partition of inverse temperature and thus we can write
\begin{equation}
