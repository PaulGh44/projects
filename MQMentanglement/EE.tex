...
\begin{aligned}
     H &= \frac{1}{2}\int \rm{d}x \,\left[\pi_\phi^2 + (\partial_x \phi)^2 + m^2 \phi^2\right] \\
       &= \frac{1}{2}\int \rm{d}x \left[\pi_\phi^2 - \phi \partial^2_x \phi + m^2 \phi^2\right]+\frac{1}{2}\left[\phi \,\partial_x\phi\right]_{x_{i}}^{x_f}\,.
\end{aligned}
\end{equation}
In absence of boundaries, the boundary term vanishes. This is the idealized situation that we should mimic in order to reproduce the situation in our setup where the system is initially defined on the infinitie line. Numerically, the field is represented by a finite dimensional vector whose size matches the size of the lattice. One way to retrieve the infinite line results is to consider the theory on the circle (or in other word, we impose periodic boundary conditions) and consider large radius with respect to the entanglement cut. Finite size effects were also studied and the entanglement entropy of a cut for the ground state for a system of size $L$ with periodic boundary conditions is reads~\cite{Calabrese_2009}~\cite{Pasquale_Calabrese_2004}
\begin{equation}\label{eq:entropysizeeffects}
  S_{A}=\frac{c}{3}\log\left(\frac{L}{\pi a}\sin\left(\frac{\pi \ell}{L}\right)\right)+c'\,,
\end{equation}
where $c'$ is a non universal contribution. This formula is actually very efficient when one wants to extract the central charge of a critical system in 1+1 dimension.
...
\paragraph{The relation between the minisuperspace action and the Klein gordon theory in 1+1d}
Consider $t=-iX$ as the time variable and $\phi$ as the space variable. Performing the substitution $w_+=e^{\phi + t}, \, w_-=e^{\phi-t}$ and defining the new variables $T = \frac{w_+ +w_-}{2},\,\bar{X} = \frac{w_+ - w_-}{2}$ we obtain the following action
\begin{equation}
\begin{aligned}
    S_{\rm{KG}} = \int \rm{d}T \rm{d}\bar{X} \frac{1}{2}\left[\left(\partial_T \Psi\right)^2 -\left(\partial_{\bar{X}} \Psi\right)^2 - \pi \mu \Psi^2\right]\,.
\end{aligned}
\end{equation}
Observe now the following correspondence. The vacuum of the original minisuperspace field theory~\eqref{eq:simpleentanglemententropy} is defined to be the unique state annhilated by the Hamiltonian which is the conserved charged associated invariance under time-translation in $t$. In other word, this is the conserved charge $H_t$ associated to the Killing vector $\xi=\partial_t$. Lines of constant $t$ are mapped to trajectories for which $T^2-X^2=\text{const}$ which describes hyperboloids. The Killing vector $\xi$ is the boost Killing vector in $T,\bar{X}$ variable as we also have $\xi \propto T\partial_T + \bar{X}\partial_{\bar{X}}$. To summurize, the pure state vacuum of the original theory is mapped to
